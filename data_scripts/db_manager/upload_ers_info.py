...
def define_tables_from_csv(metadata: MetaData, csv_file_path: str) -> Table:
    ...
    for col in csv_columns:
        columns.append(Column(col, Text if col in ['MKioskTy10Msg', 'MKioskTy12Msg', 'MKioskTy14Msg', 'MKioskTy15Msg', 'MKioskTy27Msg'] else String(255)))

    table = Table('ers_info', metadata, *columns)

    if 'hpid' in csv_columns:
        Index('idx_ers_hpid', table.c.hpid)
...
if __name__ == "__main__":
    ...
    metadata = MetaData()
    csv_file_path = "../data_sources/ers_data.csv"

    ers_info_table = define_tables_from_csv(metadata, csv_file_path)

    create_or_update_table(engine, metadata)

    df_prepared = load_and_prepare_data(csv_file_path)
    insert_data(engine, ers_info_table, df_prepared)
