...
def define_tables_from_csv(metadata: MetaData, csv_file_path: str) -> Table:
    ...
    table = Table('erh_info', metadata, *columns)

    if 'hpid' in csv_columns:
        Index('idx_erh_hpid', table.c.hpid)
...
if __name__ == "__main__":
    ...
    metadata = MetaData()
    csv_file_path = "../data_sources/erh_data.csv"
    table_name = "erh_info"

    erh_info_table = define_tables_from_csv(metadata, csv_file_path)
    ...
    insert_data(engine, erh_info_table, df_prepared)
